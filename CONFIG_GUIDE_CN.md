...
*   **QUIC 协议相关选项** (高级)
    *   **`enable_quic_proxy`** (布尔值, 可选, 默认: `false`): **(通信双方均需开启)** 启用 QUIC 代理功能的总开关。开启后，节点会尝试使用 QUIC 协议进行数据传输（如果对端支持）。QUIC 是一个现代化的、基于 UDP 的加密传输协议，旨在减少连接和传输延迟。
    *   **`disable_quic_input`** (布尔值, 可选, 默认: `false`): **(接收端配置)** 强制禁止本节点接收任何 QUIC 协议的入站连接。即使 `enable_quic_proxy` 为 `true`，此开关也会生效。
    *   **`quic_listen_port`** (整数, 可选, 默认: `0`): **(接收端配置)** 指定 QUIC 服务监听的 UDP 端口。这是一个**独立于 `listeners` 的专用端口**。只有想**接收** QUIC 连接的节点才需要将此项设置为一个**非零**的端口号（例如 `11011`）。发起端会通过路由信息自动发现此端口，无需在 `[[peer]]` 中手动指定。
